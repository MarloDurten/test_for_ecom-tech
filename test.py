...
def database_creation(client):
    ...
    client.execute('''
        CREATE TABLE IF NOT EXISTS priemniki (
            date Date,
            event_time DateTime,
            event String,
            device_id String,
...
def show_sample(client):
    rows = client.execute('SELECT * FROM priemniki LIMIT 5')  
    print("строки из таблицы:")
    for row in rows:
        print(row)
    total = client.execute('SELECT count() FROM priemniki')[0][0]  
    print(" Всего строк: ", total)

def main():
    dataf = generation(cities, dates, rng, N)  
    
    with Client('localhost', port=9000, settings={'use_numpy': True, 'connect_timeout': 30}) as client:
